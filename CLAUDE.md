...
- 確認事項: メイン4社（琉球バス・那覇バス・沖縄バス・東陽バス）のGTFS-RTがあるか
- GTFS-RTがあれば全面移行（バスナビ沖縄API廃止）、なければ併用
- 他社バス（東京バス/やんばる急行/カリー観光/沖縄エアポートシャトル）は確実にカバー
...
- 他社バス時刻表: `src/otherBusTimetable.js`（GTFS静的データから生成、`scripts/build-timetable.cjs`）
  - 30分以内の便のみ表示（宣伝っぽくならないよう制限）
  - 次の便がない路線は非表示
  - UIヘッダー: 「時刻表（定期便）」（「他社バス」は利用者に意味不明なので使わない）
