...
- バスナビ沖縄API対象外の路線バス会社（時刻表ベースで統合済み）:
  - 東京バス（TK01〜TK06）— OTTOP GTFS-RT対応（agencyId: 7011501003070）。注記: 「Google Mapsで遅延情報を確認できます」
  - カリー観光（北谷ライナー、パルコシティシャトル）— GTFS静的データ。注記: 「リアルタイム位置情報なし」
  - やんばる急行バス（YKB888/YKB3T/YKB-HLT）— 公式バスロケあり。注記: yanbaru-bus-navi.comへのリンク付き
  - 沖縄エアポートシャトル（OAS-APL/OAS-RSL/OAS-RSL-RP）— GTFS静的データ。注記: Bus-Vision（bus-viewer.jp/okinawa-shuttle/）へのリンク付き
  - ※東陽バスはメイン4社（バスナビ沖縄API対応済み）。GTFSデータも取得済み
...
- GTFS時刻表（東京バス・カリー観光・やんばる急行・沖縄エアポートシャトル）: `src/otherBusTimetable.js`（`scripts/build-timetable.cjs`で生成）
  - 方向別キー: `会社名:路線:方向インデックス`（例: `東京バス:TK02:0`下り、`東京バス:TK02:1`上り）
  - DIRECTION_META: 各方向の始発・終着バス停名を記録、方向マッチングに使用
  - flags: 1=乗車専用(降車不可)、2=降車専用(乗車不可) — pickup_type/drop_off_type対応
  - 30分以内の便のみ表示（宣伝っぽくならないよう制限）
  - 次の便がない路線は非表示
- 未出発バスのAPI登録タイミングが出発直前（数分前）→ 始発停のみ事前表示できない
  - 途中停留所は走行中バスが見えるので問題なし
  - → 始発停のみ時刻表API（Timetable）で補完済み（getTimetableBuses）
- ETA表示は出発地到着基準（目的地基準だと通過済み＝乗れないバスが表示される問題）
  - 始発停では全バスが通過済みで何も出ない → 時刻表APIで補完済み
...
- isTimetable（時刻表由来）バスの逆方向フィルタ: 行先が目的地にマッチしない場合は除外（`if (b.isTimetable) return false;`）
  - 走行中バスはrealtimeデータで正確な方向判定がされるためカバー
  - 途中停留所への未出発バスが出ない場合があるが、走行中バスでカバー
- Google Mapsリンク: 全他社バスで出発バス停→目的バス停の経路案内（東京バスも統一）
- ふりがな検索: READING_ALIASES（55件）+ 他社バス停にyomigana直接付与
...
- 那覇空港の乗り場番号: 実装済み（NAHA_AIRPORT_PLATFORMS、メイン4社+他社バス対応）
