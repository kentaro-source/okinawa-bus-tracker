...
- 那覇空港の乗り場番号: 実装済み（出発地が空港の場合、バスカードに「のりば1」等を表示）
  - NAHA_AIRPORT_PLATFORMS（api.js）: 東京バス・カリー観光→のりば1、高速・やんばる急行→のりば2、メイン4社→のりば3-4
  - メイン4社の乗り場番号も追加可能（データは公式サイト参照）

## 未解決・要検証（2026-04-12セッション）
- 方向フィルタ（routeOrder）の実地検証: 始発停で中間停留所検索が正しく動くか要確認（昼間にテスト）
- 東京バスGTFS-RTで車両位置が表示されない: Google Mapsでも非表示だったためデータソース側の問題の可能性。復旧後に再確認
- 祝日判定が未実装: YoubiKbnで曜日判定しているが祝日ダイヤ（120番等）に未対応

## 今セッションで実施した変更（2026-04-12）
- 時刻表の方向混在修正: build-timetable-api.cjs新規作成、上り/下り分離キー（東京バス:TK02:down等）
- 時刻表バスをメインリストに統合: OtherBusCard廃止、BusCard形式に変換してETA順でソート
- 「位置情報なし」表示: isScheduleOnlyフラグで区別（📋や「時刻表」タグは不要と判断し削除）
- Google Mapsリンク修正: toMapsStopName()で那覇空港→国内線旅客ターミナル前に変換
- 逆方向時刻表フィルタ: routeOrder（キャッシュv3）で方向判定、isTimetable救済にも適用
- 2時間超の未出発バス非表示: 120分閾値（沖縄は30分遅延が普通なので余裕を持たせた）
- ヘッダーに「Googleで乗換案内」リンク追加
- 那覇空港の乗り場番号: NAHA_AIRPORT_PLATFORMS実装、出発地が空港の場合バスカードに表示
- 旧build-timetable.cjs削除（build-timetable-api.cjsに置き換え済み）
